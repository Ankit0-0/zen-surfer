...
chrome.runtime.onMessage.addListener((message, sender, response) => {
  if (message.type === "BLOCK_SITES") {
    ...
    const { instaItems } = message;

    chrome.storage.sync.set({ blockedInstagramItems: instaItems }, () => {
      console.log("Blocked Instagram items: ", instaItems);
    });
  } else if (message.type === "BLOCK_YOUTUBE_ITEMS") {
    const { youtubeItems } = message;

    chrome.storage.sync.set({ blockedYoutubeItems: youtubeItems }, () => {
      console.log("Blocked Youtube items: ", youtubeItems);
    });
  }
});
